...
func NewAccountsStorage(ctx *global.CertcliOptions, domains []string) *AccountsStorage {
	...
	serverURL, err := url.Parse(ctx.Acme_server)
	if err != nil {
		klog.Fatal(err)
	}
	domain := sanitizedDomain(domains[0])

	rootPath := filepath.Join(ctx.Path, domain, baseAccountsRootFolderName)
...
func (s *AccountsStorage) LoadAccount(privateKey crypto.PrivateKey) *Account {
	...
	if err != nil {
		klog.Fatalf("Could not load file for account %s: %v", s.userID, err)
	}

	var account Account
	err = json.Unmarshal(fileBytes, &account)
	if err != nil {
		klog.Fatalf("Could not parse file for account %s: %v", s.userID, err)
	}
	...
	if account.Registration == nil || account.Registration.Body.Status == "" {
		...
		if err != nil {
			klog.Fatalf("Could not load account for %s. Registration is nil: %#v", s.userID, err)
		}

		account.Registration = reg
		err = s.Save(&account)
		if err != nil {
			klog.Fatalf("Could not save account for %s. Registration is nil: %#v", s.userID, err)
		}
	}
...
func (s *AccountsStorage) GetPrivateKey(keyType certcrypto.KeyType) crypto.PrivateKey {
	...
	if err != nil {
		klog.Fatalf("Could not load RSA private key from file %s: %v", accKeyPath, err)
	}
...
func (s *AccountsStorage) createKeysFolder() {
	if err := CreateNonExistingFolder(s.keysPath); err != nil {
		klog.Fatalf("Could not check/create directory for account %s: %v", s.userID, err)
	}
}
...
func tryRecoverRegistration(ctx *global.CertcliOptions, privateKey crypto.PrivateKey) (*registration.Resource, error) {
