...
func NewDNSHelp(o *global.CertcliOptions) *cobra.Command {
	...
	dnsHelpCmd.Flags().StringVarP(&o.Code, "code", "c", "", fmt.Sprintf("DNS code: %s", allDNSCodes()))

	return dnsHelpCmd
}

func dnsHelp(o *global.CertcliOptions) error {
	code := o.Code
	if code == "" {
	...
	return displayDNSHelp(os.Stdout, strings.ToLower(code))
}
